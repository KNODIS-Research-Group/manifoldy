...
def compute_partial_derivative_first_christoffel_symbols(ddg, l, n):
    ...
    for i in range(n):
        for j in range(n):
            for k in range(n):
                CS_first_diff[k,i,j] = 1/2*(ddg[l][j][k,i] + ddg[l][i][k,j] - ddg[l][k][i,j])
...
def compute_derivative_christoffel_symbols(g, dg, ddg):
    ...
    derivatives = []
    for l in range(n):
        CS_first_diff_l = compute_partial_derivative_first_christoffel_symbols(ddg, l,n)
        g_inv_diff_l = -np.matmul(np.matmul(g_inv,dg[l]),g_inv)
        CS_l = np.tensordot(g_inv_diff_l, CS_first, axes=([1],[0])) + np.tensordot(g_inv, CS_first_diff_l, axes=([1],[0]))
...
def compute_curvature_tensor(g, dg, ddg):
    ...
    for i in range(n):
        for j in range(n):
            for k in range(n):
                for l in range(n):
                    R[l,i,j,k] = CS_diff[j,l,i,k] - CS_diff[i,l,j,k]                     + CS_contract[i,k,l,j] - CS_contract[j,k,l,i]
...
def grid_data(X, Y, n_samples = None):
    ...
    offset = 0.05
    xs = np.array([np.linspace(0 + offset, 1 - offset,n_samples[i]) for i in range(dim)])
    ...
    return X_new.reshape(dim, n_total_samples).T, Y_new.reshape(dim, n_total_samples).T, n_samples
...
def interpolate_derivative_function(X, Y):
    ...
    for i in range(dim):
        ...
        interp = ndsplines.make_interp_spline_from_tidy(tidy_data,
    ...
    dddf = np.stack(dddf)
    dddf = np.moveaxis(dddf.reshape(dim, dim, dim, dim, n), [0,1,2,3,4],[1,2,3,4,0])
...
def estimate_metric_derivatives(X, Y, K, grid_n_samples, metric_estimation = 'KNN'):
    ...
    ddg = np.einsum('ijkls,ims->ijklm', dddf, df) + np.einsum('ikls,ijms->ijklm', ddf, ddf) + np.einsum('ijls,ikms->ijklm', ddf, ddf)\
...
def compute_norm(X, R_sq, grid_n_samples, offset, integration = 'adaptative', verbose = False):
    dim = X.shape[1]
    if integration == 'adaptative':
        ...
        for i in range(R_sq.shape[1]):
            ...
            result += integrate.nquad(f, [[offset, 1 - offset]]*dim, full_output=False)[0]
        return np.sqrt(result)
    
    elif integration == 'grid':
        reduction_indices = [range(int(grid_n_samples[i]*offset), int(grid_n_samples[i]*(1-offset))) for i in range(len(grid_n_samples))]
        ...
        return np.sqrt(quadrature_regular_grid(X_crop, R_sq_crop, grid_n_samples_crop).sum())
    else:
        print('Integration method not implemented')
        return

def L2_norm_sectional_curvature(X, Y, metric_estimation = 'KNN', K = 20, integration = 'grid',
                                grid_n_samples = None, offset = 0.25, verbose = False):
    '''
